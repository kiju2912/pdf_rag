...
1. **시작화면**  
   ![시작화면](readme/시작화면.png)  
   - **설명:** PDF 업로드 전, 시스템 초기 상태를 보여주는 메인 화면입니다. "챗팅 시작 전 PDF 등록" 버튼을 통해 PDF 파일을 업로드할 수 있습니다.

2. **일반 질문**  
   ![일반 질문](readme/일반_질문.png)  
   - **설명:** 업로드된 PDF 문서에 대해 사용자 질문을 입력할 수 있는 화면입니다. 예를 들어 “explain Figure 4”와 같은 질문을 통해 문서 내 관련 정보를 검색합니다.

3. **채팅 기본화면**  
   ![채팅 기본화면](readme/챗팅_기본화면.png)  
   - **설명:** 챗봇(질문-응답) 인터페이스의 초기 상태로, 사용자가 질문을 입력하면 채팅창에 답변이 표시됩니다.

4. **추출된 영역들**  
   ![추출된 영역들](readme/추출된_영역들.png)  
   - **설명:** PDF 파일에서 분석된 이미지 및 표, 캡션 등을 별도의 PDF/PNG 파일로 분할하여 추출한 결과입니다.

5. **탐색된 Figure 4**  
   ![탐색된 Figure 4](readme/탐색된_figure1(figure4).png)  
   - **설명:** 업로드된 PDF에서 “Figure 4”가 검색되어 해당 영역을 별도로 추출 및 PDF로 변환한 결과입니다. 관련 설명 텍스트도 함께 저장됩니다.

6. **탐색된 Figure 13**  
   ![탐색된 Figure 13](readme/탐색된_figure2(figure13).png)  
   - **설명:** “Figure 13” 부분을 찾아 별도의 PDF로 추출한 결과입니다. 사용자가 “explain Figure 13” 등의 질문 시 관련 문맥을 검색합니다.

7. **Figure 관련 질문 1**  
   ![Figure 관련 질문 1](readme/figure에_관한_질문1.png)  
   - **설명:** “Figure 4”에 대해 질문하는 장면으로, 시스템이 해당 영역과 관련된 정보를 검색하여 답변을 생성하는 과정을 보여줍니다.

8. **Figure 관련 질문 2**  
   ![Figure 관련 질문 2](readme/figure에_관한_질문2.png)  
   - **설명:** “Figure 13” 등 다른 Figure에 대해 추가 질문을 하는 화면입니다. 사용자가 구체적인 질의를 입력하면 관련 그래프 구조 및 내용에 대한 답변이 제공됩니다.
